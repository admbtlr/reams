...
export function deflateItem(item) {
  ...
  const deflated = {
    ...
    content_length:
      item.content_length ||
      (item.content_html ? stripTags(item.content_html).length : 0),
  ...
  Object.keys(deflated).forEach(
    (key) => deflated[key] === undefined && delete deflated[key]
  )
  return deflated
}
...
export function inflateStyles(item) {
  ...
  const expanded = expandStyles(
    JSON.parse(LZString.decompressFromUTF16(styles))
  )
  return {
    ...item,
    styles: expanded
...
export function fixRelativePaths(item) {
  ...
  if (item.content_mercury)
    item.content_mercury = derelativise(item.content_mercury)
...
export function sanitizeContent(item) {
  ...
  if (item.content_html)
    item.content_html = sanitizeHtml(item.content_html, settings)
  if (item.content_mercury)
    item.content_mercury = sanitizeHtml(item.content_mercury, settings)
  return item
}
...
export function addMercuryStuffToItem(item, mercury) {
  ...
  if (
    decoratedItem.content_html.length === 0 &&
  ...
  if (
    stripTags(decoratedItem.content_html) === decoratedItem.excerpt &&
    decoratedItem.content_mercury.length > 0
  ) {
    decoratedItem.showMercuryContent = true
  }
  ...
  if (
    mercuryPartial.length / htmlPartial.length > 1.3 &&
    fuzz.partial_ratio(
      htmlPartial.substring(0, 500),
      mercuryPartial.substring(0, 500)
    ) > 90
  ) {
    decoratedItem.showMercuryContent = true
  }
  ...
  if (
    decoratedItem.excerpt &&
    decoratedItem.excerpt.length > 0 &&
    fuzz.partial_ratio(decoratedItem.excerpt, htmlPartial.substring(0, 500)) >
      90
  ) {
    if (
      decoratedItem.content_html.indexOf(`<p>${decoratedItem.excerpt}</p>`) > -1
    ) {
      // excerpt is first paragraph of content_html
      decoratedItem.content_html = decoratedItem.content_html.replace(
        `<p>${decoratedItem.excerpt}</p>`,
        ''
      )
    } else if (
      decoratedItem.excerpt === htmlPartial &&
      mercuryPartial.length > 0
    ) {
      decoratedItem.showMercuryContent = true
    } else {
...
export function isExcerptExtract(item, isMercury = false) {
  ...
  const excerptWithoutEllipsis = item.excerpt.substring(
    0,
    item.excerpt.length - 4
  )
  return (
    strip(item.content_html).substring(0, excerptWithoutEllipsis.length) ===
...
export function setShowCoverImage(item) {
  const getLongestContentLength = (item) => {
    const hasMercury =
      item.content_mercury && typeof item.content_mercury === 'string'
    const hasHtml =
      item.content_html && typeof item.content_mercury === 'string'
    const mercuryLength = hasMercury ? item.content_mercury.length : 0
    const htmlLength = hasHtml ? item.content_html.length : 0
    return mercuryLength > htmlLength ? mercuryLength : htmlLength
  }
...
export function removeCachedCoverImageDuplicate(item) {
  if (
    ...
    let visibleContentKey = item.showMercuryContent
      ...
      : 'content_html'
    const firstImg =
      /<img.*?src="(.*?)".*?>/.exec(item[visibleContentKey]) &&
    ...
    if (
      ...
      fuzz.ratio(
        getImageFileName(firstImg),
        getImageFileName(item.coverImageUrl)
      ) > 70
    ) {
