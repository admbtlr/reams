...
export function useColor(host: string | undefined, shouldFetch = true) {
    ...
    const getColor = async () => {
      ...
      if (pendingAnalysis.has(host)) {
        try {
          ...
          console.error(
            `Error waiting for pending analysis for host ${host}`,
            err
          )
        }
        return
  ...
  // Return color from store if available, otherwise return state
  const hostColor = host ? hostColors.find((hc: HostColor) => hc.host === host) : undefined
  if (hostColor?.color) {
    return hostColor.color
  }
...
async function performColorAnalysis(
  ...
  let iconSource = `${process.env.EXPO_PUBLIC_API_URL}/favicon?url=https://${host}`

  let colors
  try {
    colors = await getColors(iconSource, {
      cache: true,
      key: host,
      quality: 'highest'
    })
  } catch (err) {
    console.error(`Error analyzing colors for host ${host}`, err)
    return undefined
  }
  ...
  if (colors.platform === 'ios') {
    ...
    const options: (keyof IOSImageColors)[] = [
      'background',
      'primary',
      'detail',
      'secondary'
