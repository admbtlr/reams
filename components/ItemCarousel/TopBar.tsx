...
export default function TopBar({
  ...
  const scrollRatios = useSelector((state: RootState) =>
    getScrollRatio(state, item)
  )
  ...
  if (item.isNewsletter) {
    feed = useSelector((state: RootState) =>
      state.newsletters.newsletters.find((n) => n._id === item.feed_id)
    )
  } else {
    feed = useSelector((state: RootState) =>
  ...
  useEffect(() => {
    const isCoverInline =
      (item?.styles?.isCoverInline && orientation === 'portrait') || false
  ...
  const containerStyle = useAnimatedStyle(() => {
    ...
    let inputRange = [
      pageWidth * itemIndex,
      pageWidth * (itemIndex + 0.5),
      pageWidth * (itemIndex + 1),
    ]
  ...
  const backgroundOpacityStyle = useAnimatedStyle(() => {
    const isVisible =
      Math.round(horizontalScroll.value / screenWidth) === itemIndex
  ...
  const feedTitle = feed?.title
    ...
    : !!item && item.url?.split('/').length > 3
    ? item.url.split('/')[2]
    : ''

  const getCurrentContent = (): string =>
    item.showMercuryContent ? 'mercury' : 'html'
      ...
      >
        ...
        >
          <View>
            <Favicon
              url={
                (feed && 'rootUrl' in feed ? feed.rootUrl : feed?.url) ||
                item.url
              }
            />
          </View>
          ...
          >
            ...
            <Text
              ...
              style={{
                ...
                fontFamily: filter ? 'IBMPlexSansCond-Bold' : 'IBMPlexSansCond',
                color: getForegroundColor(),
                textAlign: feed ? 'left' : 'center',
              }}
            >
              {feedTitle}
            </Text>
        ...
        >
          <Text
            style={{
              ...
              lineHeight:
                Platform.OS === 'web' ? 18 * fontSizeMultiplier() : undefined,
              fontFamily: filter ? 'IBMPlexSansCond-Bold' : 'IBMPlexSansCond',
              color: getForegroundColor(),
              textAlign: 'center',
  ...
  return isOnboarding ? null : isTitleOnly ? (
    <Reanimated.View
      key={item._id}
  ...
  return (
    <Text
      style={{
        ...getStyles().feedName,
        fontSize: 12 * fontSizeMultiplier(),
        fontFamily: filter ? 'IBMPlexSansCond' : 'IBMPlexSansCond',
        color: getForegroundColor(),
        textAlign: 'left',
      }}
    >
      {filter?.type
        ...
        ? 'Library'
        : 'Unread Stories'}{' '}
      • {itemIndex + bufferStartIndex + 1} / {numItems}
    </Text>
  )
}
