...
export default function InitialScreen({}) {
  ...
  const backend = useSelector((state: RootState) =>
    !!state.user.backends.find((b) => b.name === 'feedbin')
      ? 'feedbin'
      : 'reams'
  )
  ...
  const hasFeeds = useSelector(
    (state: RootState) => state.feeds.feeds.length > 0
  )
