...
const Title: React.FC<TitleProps> = ({
  ...
  const getInnerWidth = useCallback(
    (fontSize: number, isItalic: boolean) => {
      return (
        (screenWidth * getWidthPercentage()) / 100 -
        getMargin() * 2 -
  ...
  const getMaxFontSize = async () => {
    ...
    const widthAvailable =
      getSmallestDimension() * (getWidthPercentage() / 100) -
  ...
  let color: string = styles.isMonochrome
    ? (isFullBleed && !styles.bg) || (!isFullBleed && styles.bg)
      ...
      : textColor
    : styles.isTone
    ? item.styles?.isCoverImageColorDarker
      ? 'rgba(255,255,255,0.8)'
      : 'rgba(0,0,0,0.8)'
    : getForegroundColor()

  if (!showCoverImage || isCoverInline) {
    color = isDarkMode ? textColorDarkMode : styles.bg ? 'white' : textColor
  }
  ...
  const shadowStyle: TextStyle =
    showCoverImage && styles.hasShadow
      ? {
  ...
  const invertedTitleStyle: TextStyle = {
    color: showCoverImage ? (styles.isMonochrome ? 'black' : 'white') : color,
    paddingLeft: 2,
    paddingRight: 2,
    marginTop: 1,
  ...
  if (styles.interStyled && styles.interBolded) {
    ...
    wordStyles = styles.interBolded.map((isBold: boolean) => {
      const wordFontFamily = getFontFamily(
        isBold ? 'bold' : styles.isItalic ? 'regularItalic' : 'regular'
      )
      return {
        fontFamily: wordFontFamily,
        fontSize,
  ...
  if (shouldSplitIntoWords()) {
    finalRenderedTitle = words.map((word: string, index: number) => {
      if (styles.invertBG) {
        return (
          <View key={index} style={invertedTitleWrapperStyle}>
            <Text
