...
const FeedExpanded: React.FC<FeedExpandedProps> = ({ route }) => {
  ...
  const feed = useSelector((state: RootState) => {
    ...
    const coverImageItem = feedItems.find(item => item.hasCoverImage)

    let coverImageDimensions
    if (coverImageItem) {
      const inflatedItems = getItemsSync([coverImageItem])
      const inflatedItem = inflatedItems.length > 0 && inflatedItems[0]
      if (inflatedItem) {
        coverImageDimensions = inflatedItem.imageDimensions
      }
    }
