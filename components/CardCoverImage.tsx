...
export default function CardCoverImage({ feedId, itemId, width, height, testID }: Props) {
  ...
  const feed = useSelector((state: RootState) => feedId ? state.feeds.feeds.find(f => f._id === feedId) : undefined)
  const itemSaved = useSelector((state: RootState) => itemId ? state.itemsSaved.items.find(i => i._id === itemId) : undefined)
  const itemUnread = useSelector((state: RootState) => itemId ? state.itemsUnread.items.find(i => i._id === itemId) : undefined)
