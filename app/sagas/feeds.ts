...
export function * fetchAllFeeds () {
  ...
  newFeeds = newFeeds || []
  let toRemove = oldFeeds.filter(of => !newFeeds.find(nf => nf.feedbin_id === of.feedbin_id || nf.url === of.url))
  if (toRemove) {
    oldFeeds = oldFeeds.filter(of => !toRemove.includes(of))
  }
  newFeeds = newFeeds.filter(f => !oldFeeds
      .find(feed => feed.url === f.url || feed.feedbin_id === f.feedbin_id || feed._id === f._id))
